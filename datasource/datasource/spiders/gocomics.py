...
class GocomicsSpider(CrawlSpider):
    ...
    rules = [
        Rule(LinkExtractor(
        allow='https://www.gocomics.com/garfield/', deny=('/it/')),
        callback='parse',
        follow=True
        )
    ]
...
def parse(self, response):
    exists = response.xpath('//div[contains(@id,"comic__image")]')
    if exists:
        item = DatasourceItem()
        ...
        item['image_urls'] = response.xpath('//div[contains(@id,"comic__image")]').getall()
